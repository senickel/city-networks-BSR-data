...
wstart<-grep("<h2><span class=\"mw-headline\" id=\"Aizpute",web_page)
...
citysplit2
citysplit3<-partlist(citysplit2,2)
citysplit3<-strsplit(citysplit3,"title=\"")
citysplit3
citysplit4<-NULL
for (i in 1:length(citysplit3)) ifelse(length(citysplit3[[i]])>1,citysplit4[i]<-citysplit3[[i]][2],citysplit4[i]<-citysplit3[[i]][1])
citysplit4<-sub("</span","",citysplit4)
citysplit4<-sub("\"","",citysplit4)

citysplit4
citydata<-rbind(citysplit4)#,region) ## bind city and region to dataset
...
head(partnersplit)
partnersplit[72]
partnersplit2<-paste(partlist(partnersplit,4),partlist(partnersplit,5))
partnersplit2
...
citycountry2[which(citycountry2[,2]=="Texas, United States"),2]<-"United States"
